...
#            Run unpacked types example ..... doit.sh run
...
if [ "$1" != "run" ] ; then
    ...
    echo "### Run unpacked types example ..... doit.sh run"
...
echo ""
echo "### NOTE: Running DPI unpacked types example ..."
echo ""
vlog -sv -dpiheader dpi_array_xor.h dpi_array_xor.v
$CC dpi_array_xor.c
case $platform in
Win*|CYGWIN_NT*)
    $LD dpi_array_xor.obj 
    ;;
*)
    $LD dpi_array_xor.so dpi_array_xor.o
    ;;
esac
vsim -c top -sv_lib dpi_array_xor -do "run -all; quit -f"
