...
#            Run DPI/VPI example ............ doit.sh run
...
if [ "$1" != "run" ] ; then
	...
	echo "### Run DPI/VPI example ............ doit.sh run"
...
echo ""
echo "### NOTE: Running DPI-VPI linkage example ..."
echo ""
vlog -sv -dpiheader $testname.h $testname.sv top.sv
$CC $testname.c
case $platform in
Win*|CYGWIN_NT*)
    $LD $testname.obj 
...
vsim -c -voptargs="+acc=rnl" top -sv_lib $testname -do sim.do
exit 0
